...
VKAPI_ATTR uint32_t VKAPI_CALL VulkanDevice::debug_callback(
...
VkResult VulkanDevice::create_debug_utils_messenger_ext(
    VkInstance instance,
...
void VulkanDevice::destroy_debug_utils_messenger_ext(
    VkInstance instance,
...
bool VulkanDevice::is_device_suitable(VkPhysicalDevice device) noexcept
{
    ...
    bool suitable = indices.is_complete() &&
        extensionsSupported &&
        swapChainAdequate &&
...
DeviceLocalMemoryBudget VulkanDevice::get_device_local_memory_budget() const noexcept
{
    ...
    if (!m_hasMemoryBudgetExtension || m_instance == nullptr || m_physicalDevice == nullptr)
        return result;
...
QueueFamilyIndices VulkanDevice::find_queue_families(VkPhysicalDevice device) noexcept
{
    ...
    for (const auto& queueFamily : queueFamilies)
    {
        ...
        std::uint32_t presentSupport{false};
...
Result<std::uint32_t> VulkanDevice::find_memory_type(std::uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
...
Result<> VulkanDevice::create_buffer(
    VkDeviceSize size,
...
Result<> VulkanDevice::create_image(
    std::uint32_t width,
    std::uint32_t height,
    VkFormat format,
...
Result<> VulkanDevice::transition_image_layout(
    VkImage image,
    VkFormat format,
...
)
{
    ...
    if (!commandBufferResult)
        return make_error(
            "Failed to begin command buffer for layout transition",
...
Result<> VulkanDevice::copy_buffer_to_image(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height)
{
    ...
    if (!commandBufferResult)
        return make_error(
            "Failed to begin command buffer for buffer-to-image copy",
