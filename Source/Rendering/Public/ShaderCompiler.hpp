...
class NOC_EXPORT ShaderCompiler
{
  public:
    ...
    static Result<std::vector<uint32_t>> compile(std::string_view glslSource, ShaderStage stage,
    ...
    static Result<std::vector<uint32_t>> compile_file(const std::filesystem::path& glslPath);
    ...
    static Result<std::vector<uint32_t>> compile_or_cache(const std::filesystem::path& glslPath,
  ...
  private:
    ...
    /// Reads a .spv binary file into a uint32_t vector.
    static Result<std::vector<uint32_t>> read_spv(const std::filesystem::path& spvPath);

    /// Writes SPIR-V data to a binary file.
    static Result<> write_spv(const std::filesystem::path& spvPath, const std::vector<uint32_t>& spirv);
};
