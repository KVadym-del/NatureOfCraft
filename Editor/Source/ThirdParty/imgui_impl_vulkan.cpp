...
//  init_info.PipelineInfoMain.RenderPass, init_info.Subpass --> init_info.PipelineInfoMain.Subpass,
//  init_info.MSAASamples --> init_info.PipelineInfoMain.MSAASamples, init_info.PipelineRenderingCreateInfo -->
//  init_info.PipelineInfoMain.PipelineRenderingCreateInfo. 2025-09-26: *BREAKING CHANGE*: renamed
//  ImGui_ImplVulkan_MainPipelineCreateInfo to ImGui_ImplVulkan_PipelineInfo. Introduced very recently so shouldn't
//  affect many users. 2025-09-26: *BREAKING CHANGE*: helper ImGui_ImplVulkanH_CreateOrResizeWindow() added a
//  VkImageUsageFlags image_usage` argument, default to VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT if 0. 2025-09-26: Vulkan:
//  Added a way to customize shaders by filling
//  ImGui_ImplVulkan_InitInfo::CustomShaderVertCreateInfo/CustomShaderFragCreateInfo. (#8585) 2025-09-18: Call
//  platform_io.ClearRendererHandlers() on shutdown. 2025-09-04: Vulkan: Added ImGui_ImplVulkan_CreateMainPipeline().
//  (#8110, #8111) 2025-07-27: Vulkan: Fixed texture update corruption introduced on 2025-06-11. (#8801, #8755, #8840)
//  2025-07-07: Vulkan: Fixed texture synchronization issue introduced on 2025-06-11. (#8772)
//  2025-06-27: Vulkan: Fixed validation errors during texture upload/update by aligning upload size to
//  'nonCoherentAtomSize'. (#8743, #8744) 2025-06-11: Vulkan: Added support for ImGuiBackendFlags_RendererHasTextures,
//  for dynamic font atlas. Removed ImGui_ImplVulkan_CreateFontsTexture() and ImGui_ImplVulkan_DestroyFontsTexture().
...
static VkPipeline ImGui_ImplVulkan_CreatePipeline(VkDevice device, const VkAllocationCallbacks* allocator,
                                                  ...
                                                  const ImGui_ImplVulkan_PipelineInfo* info)
{
    ...
    {
        IM_ASSERT(info->PipelineRenderingCreateInfo.sType == VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR &&
...
#ifdef IMGUI_IMPL_VULKAN_HAS_DYNAMIC_RENDERING
    create_main_pipeline |= (v->UseDynamicRendering && v->PipelineInfoMain.PipelineRenderingCreateInfo.sType ==
...
// etc.). You may read this code to learn about Vulkan, but it is recommended you use your own custom tailored code to
// do equivalent work. (The ImGui_ImplVulkanH_XXX functions do not interact with any of the state used by the regular
