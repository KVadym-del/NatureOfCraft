...
struct ImGui_ImplVulkan_InitInfo
{
    ...
    uint32_t DescriptorPoolSize;     // Optional: set to create internal descriptor pool automatically instead of using
                                     // DescriptorPool.
    ...
    // --> Since 2025/09/26: set 'PipelineInfoMain.MSAASamples' instead VkPipelineRenderingCreateInfoKHR
    // PipelineRenderingCreateInfo; // Since 2025/09/26: set 'PipelineInfoMain.PipelineRenderingCreateInfo' instead

    // (Optional) Dynamic Rendering
    // Need to explicitly enable VK_KHR_dynamic_rendering extension to use this, even for Vulkan 1.3 + setup
    ...
    // - Shader inputs/outputs need to match ours. Code/data pointed to by the structure needs to survive for whole
    // during of backend usage.
...
// The main window pipeline will be created by ImGui_ImplVulkan_Init() if possible (== RenderPass xor
// (UseDynamicRendering && PipelineRenderingCreateInfo->sType == VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR))
